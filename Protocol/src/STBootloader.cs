...
namespace Lytec.Protocol
{
    public class STBootloader
    {
        ...
        public static IDictionary<ushort, DeviceInfo> DeviceInfos { get; set; } = DeviceInfo.BuiltInList.ToDictionary(v => v.DeviceId);
        ...
        //     其他 定长数据
        public static bool Exec(ConfigEx conf, Command cmd, out byte[] Answer, int answerLen, int extTimeout = 0)
        => Exec(conf, new byte[] { (byte)cmd }, out Answer, answerLen, extTimeout, CheckSumType.Xor_Init0xFF);

        // answerLen:
        //      0   无数据
        //      -1  变长数据, 数据中的第1字节为数据长度
        //     其他 定长数据
        public static bool WaitAnswer(ConfigEx conf, out byte[] Answer, int answerLen, int extTimeout = 0, bool ackAfterData = true)
        => Exec(conf, null, out Answer, answerLen, extTimeout, ackAfterData: ackAfterData);

        // answerLen:
        //      0   无数据
        //      -1  变长数据, 数据中的第1字节为数据长度
        //     其他 定长数据
        public static bool Exec(ConfigEx conf, byte[] data, out byte[] Answer, int answerLen, int extTimeout = 0, CheckSumType chkSum = CheckSumType.Xor_Init0x00, bool ackAfterData = true)
        {
            Answer = null;
            switch (conf.Config)
            {
                //case SendAndGetAnswerConfig exeConf:
                //    break;
                case Configs conf1:
                    using (var comm = new ClientCommunicator(conf1))
                        return exec(comm, out Answer);
                case IClientCommunicator comm:
                    return exec(comm, out Answer);
                default:
                    throw new NotImplementedException();
            }
            bool exec(IClientCommunicator comm, out byte[] answer)
            {
                answer = null;
                var sbuf = Array.Empty<byte>();
                if (data != null && data.Length > 0)
                {
                    sbuf = data;
                    switch (chkSum)
                    {
                        case CheckSumType.Xor_Init0x00:
                        case CheckSumType.Xor_Init0xFF:
                            sbuf = data.Append(GetCheckSum(data, chkSum == CheckSumType.Xor_Init0x00 ? 0 : 0xFF)).ToArray();
                            break;
                    }
                }
                for (var retryCount = -1; retryCount < comm.CurrentConfig.Retries; retryCount++)
                {
                    try
                    {
                        if (sbuf.Length > 0)
                        ...
                        {
                            var answer1 = comm.Receive(200 + extTimeout);
                            if (answer1 == null || answer1.Length < 1)
                                continue;
                            if (sbuf.Length < 1)
                            {
                                foreach (var b in answer1)
                                    rbuf.Enqueue(b);
                                if (answerLen < 0)
                                    answerLen = rbuf.Dequeue();
                                if (rbuf.Count >= answerLen)
                                {
                                    answer = rbuf.Take(answerLen).ToArray();
                                    return true;
                                }
                                continue;
                            }
                            for (var i = 0; i < answer1.Length; i++)
                            {
                                switch (answer1[i])
                                {
                                    case (byte)SpBytes.ACK:
                                        if (answerLen == 0)
                                            return true;
                                        for (var j = i + 1; j < answer1.Length; j++)
                                            rbuf.Enqueue(answer1[j]);
                                        while (DateTime.Now < rTimeout)
                                        {
                                            if (rbuf.Count < answerLen)
                                            {
                                                ...
                                                foreach (var b in comm.Receive())
                                                    rbuf.Enqueue(b);
                                            }
                                            if (rbuf.Count > 0 && answerLen == -1)
                                                answerLen = rbuf.Dequeue() + 1;
                                            if (ackAfterData)
                                            {
                                                if (rbuf.Count > answerLen)
                                                {
                                                    if (rbuf.Skip(answerLen).First() == (byte)SpBytes.ACK)
                                                    {
                                                        answer = rbuf.Take(answerLen).ToArray();
                                                        return true;
                                                    }
        ...
        public static bool GetBootloaderInfo(ConfigEx conf, out BootloaderInfo Info, int extTimeout = 0)
        {
            Info = null;
            if (!Exec(conf, Command.Get, out var data, -1, extTimeout: extTimeout))
                return false;
            Info = new BootloaderInfo(data[0], data.Skip(1).ToArray());
            return true;
        }

        public static bool GetVersionAndReadProtectionStatus(ConfigEx conf, out Version Version, out ReadProtectionStatus Status, int extTimeout = 0)
        {
            Version = null;
            Status = null;
            if (!Exec(conf, Command.GetVersionAndReadProtectionStatus, out var data, 3, extTimeout: extTimeout))
                return false;
        ...
        public static bool GetChipProductId(ConfigEx conf, out ushort ProductId, out byte[] ProductIdBytes, int extTimeout = 0)
        {
            ProductId = 0;
            if (!Exec(conf, Command.GetChipProductId, out ProductIdBytes, -1, extTimeout: extTimeout))
                return false;
        ...
        public static bool Read(ConfigEx conf, int address, int length, out byte[] Data, Action<OnProcessEventArgs> onProcess = null, int extTimeout = 0)
        {
            Data = null;
            ...
            for (int offset = 0, plen; offset < rlen; offset += plen)
            {
                ...
                if (!Exec(conf, addr.ToBytes(Endian.Big), out _, 0, extTimeout))
                    return false;
                if (!Exec(conf, new byte[] { (byte)(plen - 1) }, out var d, plen, extTimeout, CheckSumType.Xor_Init0xFF, false))
                    return false;
                buf.AddRange(d);
                if (rlen > MaxReadStepSize)
        ...
        public static bool ReadInt(ConfigEx conf, int address, out int Value, Action<OnProcessEventArgs> onProcess = null, int extTimeout = 0)
        {
            Value = -1;
            if (!Read(conf, address, 4, out var bytes, onProcess, extTimeout))
                return false;
            Value = bytes.ToStruct<int>(Endian.Little);
            return true;
        }

        public static bool GetFlashSize(ConfigEx conf, DeviceInfo info, out int FlashSize, int extTimeout = 0)
        => GetFlashSize(conf, info.FlashSizeAddress, out FlashSize, extTimeout);

        public static bool GetFlashSize(ConfigEx conf, int address, out int FlashSize, int extTimeout = 0)
        {
            if (!ReadInt(conf, address, out FlashSize, null, extTimeout))
                return false;
            // 数据取低16位, 单位为KB
        ...
        public static bool Go(ConfigEx conf, int address, int extTimeout = 0)
        {
            if (!Exec(conf, Command.Go, out _, 0, extTimeout))
                return false;
            if (!Exec(conf, address.ToBytes(Endian.Big), out _, 0, extTimeout + 1000))
                return false;
            if (WaitAnswer(conf, out var ack, 1, extTimeout + 1000, false))
                return ack[0] == (byte)SpBytes.ACK;
            return true;
        }

        public static bool Reset(ConfigEx conf, DeviceInfo info)
        ...
        public static bool Write(ConfigEx conf, int address, IEnumerable<byte> data, int extTimeout = 0, Action<OnProcessEventArgs> onProcess = null)
        => WriteAligned(conf, address, data.ToArray(), extTimeout, onProcess);
        public static bool WriteAligned(ConfigEx conf, int address, byte[] data, int extTimeout = 0, Action<OnProcessEventArgs> onProcess = null, int addrAlign = WriteAddressAlign, int sizeAlign = WriteSizeAlign)
        {
        ...
        private static bool Erase(ConfigEx conf, IEnumerable<int> pageIndexes, int extTimeout = 0)
        {
        ...
        private static bool ExtendedErase(ConfigEx conf, IEnumerable<int> pageIndexes, int extTimeout = 0)
        {
        ...
        public static bool ExtendedErase(ConfigEx conf, ExtendedEraseBank bank, int extTimeout = 0)
        {
            if (!Exec(conf, Command.ExtendedErase, out _, 0, extTimeout))
                return false;
        ...
        public static bool FullChipErase(ConfigEx conf, bool useExtendedErase, int extTimeout = 0)
        {
            if (useExtendedErase)
            ...
            else
            {
                if (!Exec(conf, Command.Erase, out _, 0, extTimeout))
                    return false;
                return Exec(conf, new byte[] { 0xFF }, out _, 0, extTimeout + 500, CheckSumType.Xor_Init0xFF);
            }
        }

        public static bool Erase(ConfigEx conf, DeviceInfo info, bool useExtendedErase, int address, int length, int extTimeout = 0)
        => InnerErase(conf, info, useExtendedErase, address, length, false, null, extTimeout);

        public static bool SafeErase(ConfigEx conf, DeviceInfo info, bool useExtendedErase, int address, int length, Action<OnProcessEventArgs> onProcess = null, int extTimeout = 0)
        => InnerErase(conf, info, useExtendedErase, address, length, true, onProcess, extTimeout);

        private static bool InnerErase(ConfigEx conf, DeviceInfo info, bool useExtendedErase, int address, int length, bool keepAlignData, Action<OnProcessEventArgs> onProcess = null, int extTimeout = 0)
        {
            ...
            else
            {
                // 非统一的页大小
                var startPage = 0;
                int GetFlashPageSize(int pageIndex) => pageSizes[Math.Min(pageSizes.Length, pageIndex)];
        ...
        public static bool WriteProtect(ConfigEx conf, IEnumerable<int> pageIndexes, int extTimeout = 0)
        {
        ...
        public static bool WriteUnprotect(ConfigEx conf, int extTimeout = 0)
        {
            if (!Exec(conf, Command.WriteUnprotect, out _, 0, extTimeout))
                return false;
            if (WaitAnswer(conf, out var ack, 1, extTimeout, false))
                return ack[0] == (byte)SpBytes.ACK;
            return true;
        }

        public static bool ReadoutProtect(ConfigEx conf, int extTimeout = 0)
        {
        ...
        public static bool ReadoutUnprotect(ConfigEx conf, int extTimeout = 0)
        {
            if (!Exec(conf, Command.ReadoutUnprotect, out _, 0, extTimeout))
                return false;
            if (WaitAnswer(conf, out var ack, 1, extTimeout + 10000, false))
                return ack[0] == (byte)SpBytes.ACK;
            return true;
        }
    }
